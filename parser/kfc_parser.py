...
def get_data(html):
    ...
    items = soup.find_all('div', class_="Flipper_Flipper__1iR3F Flipper_Flipper__paper__1OvF7 ProductCard ProductList_ProductList__item__3UmRR")
    products = []
    counter = 0
    for item in items:
        counter += 1
        ingredients = item.find(class_='Subheading_Subheading__1MXqs ProductCard_ProductCard__subheading__2uYmw')
        price = item.find(class_ = 'Amount_Amount__1zPkL Amount_Amount__price__dFaAo ProductCard_ProductCard__price__2WUWl Amount_Amount_size_l__1qY9g')
